...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QLineEdit, QCheckBox, QPushButton, QMessageBox, QGridLayout, \
    QGroupBox, QHBoxLayout, QScrollArea, QFileDialog
...
class MainWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.button_avg.setToolTip(
            'Output the min/max/avg of the selected rails to a csv file in the same directory as the data file')
        self.button_avg.clicked.connect(self.avgOutput)
        self.button_clipboard = QPushButton('Copy avg to clipboard', self)
        self.button_clipboard.setToolTip(
            'Copy the average of the selected rails in order to the clipboard. Press Ctrl+V to Paste.')
    ...
    def createRailList(self):
        ...
        if not self.config_file_content_detail:  # No Rails
            ...
            data_raw = pd.read_csv(str(self.le_datafilename.text()), header=1, index_col=0, skiprows=[2],
                                   nrows=2)  # Read the data file

            for x in xrange(1, len(data_raw.iloc[0])):
                # Parse the Rail names
                config_railname = data_raw.columns[x].split('=',
                                                            1)  # Rails name are in the form "RAILNAME = FORMULA" or "RAILNAME". Split at '='.
            ...
            data_raw = pd.read_csv(str(self.le_datafilename.text()), header=1, index_col=0, skiprows=[2], nrows=2)
            if len(data_raw.iloc[0]) == len(self.config_file_content_detail):  # Same numbers of rails. Same rails.
                ...
                for x in xrange(0, len(checked_rails)):
                    try:
                        location = temp.index(checked_rails[
                                                  x])  # If the old rail exists in the new data file. Set boolean to TRUE. Otherwise jump to next iteration.
                    except:
                        continue
                    else:
    ...
    def mainPlot(self):
        ...
        """ Main Script """
        data_raw = pd.read_csv(data_filename, header=1, index_col=0, skiprows=[2])
        ...
        # Plot Rails
            ...
            if len(data_file_content_parsed) <= 1:
                ...
                return
            if self.useMV:
                data_content_MV = data_moving_average(data_file_content_parsed, config_file_content_parsed, MVinterval,
                                                      MVstep)
                plt.ion()
                data_plot(data_content_MV, config_file_content_parsed, data_filename, figure_name, self.stack,
                          self.data_filename_changed, self.stack_changed, self.MV_change, self.useFN)
            else:
                plt.ion()
                data_plot(data_file_content_parsed, config_file_content_parsed, data_filename, figure_name, self.stack,
                          self.data_filename_changed, self.stack_changed, self.MV_change, self.useFN)
            plt.draw()
            self.stack_changed = False
            self.MV_change = False
...
def data_plot(data_file_content_parsed, config_file_content_parsed, data_filename, figure_name, stack_graph=False,
              data_filename_changed=False, stack_graph_change=False, MV_change=False, FN_enable=False):
...
def data_average(data_file_content_parsed_col):
    if type(
            data_file_content_parsed_col) is not list:  # Moving average calls this function and with data_file_content_parsed_col being a list
